...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf User ke banaye hue Orders ko handle karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t order-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`order-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8005"]`** 
   - *(Service ko command diya ki aaram se port 8005 par startup ho jao).*
...
Jab bhi API Gateway ko kisika Order place karna hoga, wo chup chap Kubernetes ke andar `http://order-service:8005` bulakar kaam nikaal lega.
