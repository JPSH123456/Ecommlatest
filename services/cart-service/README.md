...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf User ki Shopping Cart chizo ko handle karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t cart-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`cart-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8004"]`** 
   - *(Service ko command diya ki aaram se port 8004 par startup ho jao).*
...
Jab bhi API Gateway ko cart ki items jani hongi, wo chup chap Kubernetes ke andar `http://cart-service:8004` bulakar apka data utha lega.
