...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf Products (Items, Catalog, Stocks) se judi chizo ko handle karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t product-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`product-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8003"]`** 
   - *(Service ko command diya ki aaram se port 8003 par startup ho jao).*
...
Jab bhi API Gateway ko Catalog dikhana hoga, wo chup chap Kubernetes ke andar `http://product-service:8003` bulakar apka data utha lega. Isiliye hume isey public Host ya Path dekar duniya ko expose karne ki koi jaroorat hi nahi!
