...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf Products par padne wale Reviews aur Ratings ko handle karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t review-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`review-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8007"]`** 
   - *(Service ko command diya ki aaram se port 8007 par startup ho jao).*
...
Jab bhi User kisi product pe review dikhane ko kahega, toh API Gateway internally `http://review-service:8007` ke paas jaayega aur review nikal laega!
