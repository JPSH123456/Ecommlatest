...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf User Authentication (Login, Register, Token banane) se judi chizo ko handle karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t auth-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`auth-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8001"]`** 
   - *(Service ko command diya ki aaram se port 8001 par startup ho jao).*
...
Jab bhi API Gateway ko login verify karna hoga, wo chup chap Kubernetes ke andar `http://auth-service:8001` bulakar apka data utha lega. Isiliye hume isey public Host ya Path dekar duniya ko expose karne ki koi jaroorat hi nahi!
