...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf User ki Wishlist, yaani usne future ke liye kya products bachakar rakhe hain unhe record rakhna hai.

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t wishlist-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`wishlist-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8008"]`** 
   - *(Service ko command diya ki aaram se port 8008 par startup ho jao).*
...
Jab koi user apni wishlist load karega, toh yahan bahar se koi request nahi aa sakti, API Gateway chup chap aakar `http://wishlist-service:8008` se baat karega aur information la dega!
