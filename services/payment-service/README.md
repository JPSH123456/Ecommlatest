...
Bhai! Ye ek Backend (Internal) service hai. Iska kaam sirf aur sirf Payments ki processing aur invoices create karna hai. 

## 🐳 Docker Image Kis Naam se Banegi?
Aap terminal mein jaakar command chalaoge:
`docker build -t payment-service:latest .`
Toh aapki image ban jayegi jiska naam hamesha **`payment-service:latest`** hoga.
...
8. **`CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8006"]`** 
   - *(Service ko command diya ki aaram se port 8006 par startup ho jao).*
...
Jab bhi API Gateway ko kisika Invoice banana hoga ya Payment confirm karni hogi, wo chup chap Kubernetes ke andar `http://payment-service:8006` ko message bhej dega.
